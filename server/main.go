...
import (
	...
	"github.com/paw-digital/Animal/server/controller"
...
func CorsMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if origin == "" {
			origin = "https://animal.paw.digital"
		}
...
func main() {
	// Get seed from env
	seed := utils.GetEnv("ANIMAL_SEED", "1234567890")
	...
	// Address
	apiGroup.GET("/animal/:address", animalController.GetAnimal)
	apiGroup.POST("/animal/dtl", animalController.AnimalStats)
	// Testing
	if gin.IsDebugging() {
		apiGroup.GET("/random", animalController.GetRandomSvg)
	}
